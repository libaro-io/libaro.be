...
class ShowcaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (request()->has('filter')) {
            ...
            $showcases = Showcase::where('visible', 1)->withAnyTags($filters)->orderBy('date', 'desc')->orderBy('id', 'asc')->paginate(6);
        } else {
            $filters = null;
            $showcases = Showcase::where('visible', 1)->orderBy('date', 'desc')->orderBy('id', 'asc')->paginate(6);
        }
